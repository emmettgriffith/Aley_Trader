"""ai_patterns.py
AI-assisted pattern recognition utilities for time-series (finance or sensors).

Features:
  - Technical indicators (SMA, EMA, RSI, MACD, Bollinger Bands, ATR)
  - Rule-based signals (MA crossovers, RSI zones, breakouts, volatility regimes)
  - Anomaly detection via IsolationForest
  - A pluggable ML pipeline (scikit-learn) for supervised pattern classification
  - Clean, typed APIs designed to integrate with pandas DataFrames

Dependencies:
  numpy, pandas, scikit-learn, scipy (optional but recommended)
"""
...
# -----------------------------
def detect_anomalies_isoforest(df: pd.DataFrame, feature_cols: List[str],
                               contamination: float = 0.02, random_state: int = 7) -> pd.Series:
    ...
    pred = pd.Series(model.predict(X), index=X.index)  # -1 anomaly, 1 normal
    outlier = pred.eq(-1)
    # align back to df index
    out = pd.Series(False, index=df.index, name="anomaly")
    out.loc[outlier.index] = outlier.values
    return out
...
def analyze_patterns(df: pd.DataFrame,
    ...
    # Anomaly detection on engineered features
    feats = compute_features(df, price_col, high_col, low_col, volume_col)
    feature_cols = [c for c in feats.columns if feats[c].dtype != "O"]
    signals["anomaly"] = detect_anomalies_isoforest(feats, feature_cols, config.anomaly_contamination)

    return signals

# -----------------------------
# Supervised ML example (pluggable)
# -----------------------------
def train_classifier_labeled(df: pd.DataFrame, feature_cols: List[str], label_col: str, test_size: float = 0.2,
                             random_state: int = 42) -> Tuple[Pipeline, str]:
    X = df[feature_cols].replace([np.inf, -np.inf], np.nan).dropna()
    ...
    pipe = Pipeline([
        ("scaler", StandardScaler(with_mean=False)),  # works with sparse-like data too
