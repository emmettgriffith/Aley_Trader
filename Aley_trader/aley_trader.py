...
def show_chart_with_points(symbol, ticker, prev_close, latest_close, percent_gain, ta_signal, rsi, analysis, notebook, tab_title):
    ...
    def draw_chart(chart_type="Candlestick"):
        ...
        elif chart_type.startswith("AI"):
            # Render AI pattern analysis charts
            if not AI_PATTERNS_AVAILABLE:
                error_label = tk.Label(frame, 
                    text="AI Patterns module not available. Please install: pip install scikit-learn scipy", 
                    bg=DEEP_SEA_THEME['primary_bg'], fg=DEEP_SEA_THEME['danger'], font=("Segoe UI", 12))
                error_label.pack(expand=1)
                return
                
            try:
                # Prepare data for AI pattern analysis
                # Convert matplotlib date numbers back to datetime
                if 'Date' in chart_hist.columns:
                    dates = pd.to_datetime(chart_hist['Date'], unit='D', origin='1970-01-01')
                else:
                    dates = pd.date_range('2024-01-01', periods=len(chart_hist), freq='D')
                
                ai_df = pd.DataFrame({
                    'close': chart_hist['Close'],
                    'high': chart_hist['High'],
                    'low': chart_hist['Low'],
                    'volume': chart_hist.get('Volume', chart_hist['Close'] * 0 + 1000)  # Default volume if not available
                }, index=dates)
                
                # Run AI pattern analysis
                config = ai_patterns.PatternConfig(
                    fast_ma=10,
                    slow_ma=30,
                ...
                signals = ai_patterns.analyze_patterns(ai_df, config=config)
                
                # Create the chart based on specific AI chart type
                plt.close('all')
                ...
                ax_price, ax_signals, ax_features = axes
                
                # Plot price data
                ax_price.plot(ai_df.index, ai_df['close'], color=DEEP_SEA_THEME['info'], linewidth=1.5, label='Close Price')
                
                if chart_type == "AI Pattern Analysis":
                    # Show all patterns
                    signal_colors = {
                        'ma_bull_cross': DEEP_SEA_THEME['success'],
                        'rsi_overbought': DEEP_SEA_THEME['danger'], 
                        'rsi_oversold': DEEP_SEA_THEME['active'],
                        'breakout': DEEP_SEA_THEME['warning'],
                        'anomaly': DEEP_SEA_THEME['text_accent']
                    }
                    
                    for signal_name, signal_series in signals.items():
                        if signal_name in signal_colors and signal_series.any():
                            signal_points = ai_df[signal_series]
                            if len(signal_points) > 0:
                                ax_price.scatter(signal_points.index, signal_points['close'], 
                                              color=signal_colors[signal_name], s=30, alpha=0.7,
                                              label=signal_name.replace('_', ' ').title())
                    
                elif chart_type == "AI Anomaly Detection":
                    # Focus on anomaly detection
                    anomaly_points = ai_df[signals['anomaly']]
                    if len(anomaly_points) > 0:
                        ax_price.scatter(anomaly_points.index, anomaly_points['close'],
                                       color=DEEP_SEA_THEME['danger'], s=50, alpha=0.8,
                                       label='Anomalies', marker='X')
                    
                elif chart_type == "AI MA Crossover Signals":
                    # Focus on moving average crossovers
                    crossover_points = ai_df[signals['ma_bull_cross']]
                    if len(crossover_points) > 0:
                        ax_price.scatter(crossover_points.index, crossover_points['close'],
                                       color=DEEP_SEA_THEME['success'], s=40, alpha=0.8,
                                       label='Bullish MA Crossover', marker='^')
                    
                    # Add moving averages
                    ma_fast = ai_patterns.sma(ai_df['close'], config.fast_ma)
                    ma_slow = ai_patterns.sma(ai_df['close'], config.slow_ma)
                    ax_price.plot(ai_df.index, ma_fast, color=DEEP_SEA_THEME['text_accent'], 
                                linewidth=1, alpha=0.7, label=f'MA{config.fast_ma}')
                    ax_price.plot(ai_df.index, ma_slow, color=DEEP_SEA_THEME['warning'], 
                                linewidth=1, alpha=0.7, label=f'MA{config.slow_ma}')
                    
                elif chart_type == "AI Breakout Detection":
                    # Focus on breakouts
                    breakout_points = ai_df[signals['breakout']]
                    if len(breakout_points) > 0:
                        ax_price.scatter(breakout_points.index, breakout_points['close'],
                                       color=DEEP_SEA_THEME['warning'], s=40, alpha=0.8,
                                       label='Breakouts', marker='D')
                
                # Style main chart
                ax_price.set_facecolor(DEEP_SEA_THEME['primary_bg'])
                ax_price.tick_params(colors=DEEP_SEA_THEME['text_primary'])
                ax_price.set_ylabel("Price", color=DEEP_SEA_THEME['text_primary'])
                ax_price.set_title(f"{symbol} - {chart_type} ({current_timeframe.get().upper()})", 
                                 color=DEEP_SEA_THEME['text_primary'], fontsize=14)
                ax_price.grid(True, color=DEEP_SEA_THEME['border'], linestyle='-', linewidth=0.3)
                ax_price.legend(loc='upper left', facecolor=DEEP_SEA_THEME['secondary_bg'], 
                              edgecolor=DEEP_SEA_THEME['border'])
                
                # Signal frequency chart
                signal_counts = {name: series.sum() for name, series in signals.items()}
                signal_names = list(signal_counts.keys())
                signal_values = list(signal_counts.values())
                
                bars = ax_signals.bar(range(len(signal_names)), signal_values, 
                                    color=DEEP_SEA_THEME['info'], alpha=0.7)
                ax_signals.set_xticks(range(len(signal_names)))
                ax_signals.set_xticklabels([name.replace('_', '\n') for name in signal_names], 
                                         rotation=45, ha='right', fontsize=8)
                ax_signals.set_ylabel("Count", color=DEEP_SEA_THEME['text_primary'], fontsize=10)
                ax_signals.set_title("Signal Frequency", color=DEEP_SEA_THEME['text_primary'], fontsize=12)
                ax_signals.set_facecolor(DEEP_SEA_THEME['primary_bg'])
                ax_signals.tick_params(colors=DEEP_SEA_THEME['text_primary'])
                ax_signals.grid(True, color=DEEP_SEA_THEME['border'], linestyle='-', linewidth=0.3)
                
                # Feature plot (RSI)
                try:
                    rsi_values = ai_patterns.rsi(ai_df['close'], 14)
                    ax_features.plot(ai_df.index, rsi_values, color=DEEP_SEA_THEME['text_accent'], linewidth=1)
                    ax_features.axhline(70, color=DEEP_SEA_THEME['danger'], linestyle='--', alpha=0.7)
                    ax_features.axhline(30, color=DEEP_SEA_THEME['success'], linestyle='--', alpha=0.7)
                    ax_features.set_ylabel("RSI", color=DEEP_SEA_THEME['text_primary'], fontsize=10)
                    ax_features.set_ylim(0, 100)
                except Exception:
                    ax_features.text(0.5, 0.5, 'Feature calculation error', 
                                   transform=ax_features.transAxes, ha='center',
                                   color=DEEP_SEA_THEME['text_secondary'])
                
                ax_features.set_facecolor(DEEP_SEA_THEME['primary_bg'])
                ax_features.tick_params(colors=DEEP_SEA_THEME['text_primary'])
                ax_features.grid(True, color=DEEP_SEA_THEME['border'], linestyle='-', linewidth=0.3)
                ...
                print(f"AI Pattern Analysis completed for {symbol}")
                print(f"  Pattern signals detected:")
                for signal_name, count in signal_counts.items():
                    percentage = (count / len(ai_df)) * 100 if len(ai_df) > 0 else 0
                    print(f"    {signal_name:20}: {count:3d} ({percentage:5.1f}%)")
                
                status_label.config(text=f"OK {chart_type} analysis complete")
                return
                
            except Exception as e:
                print(f"Error creating AI pattern chart: {e}")
                error_label = tk.Label(frame, text=f"AI Analysis Error: {e}", 
                                     bg=DEEP_SEA_THEME['primary_bg'], fg=DEEP_SEA_THEME['danger'], 
                                     font=("Segoe UI", 12))
                error_label.pack(expand=1)
                return
...
def main_tabbed_chart():
    ...
    def create_home_page():
        ...
        def make_card(parent, title, subtitle="", width=340, height=150):
            ...
            # Resize handler so cards grow with the window
            def _on_resize(e):
                w = max(160, e.width)
                h = max(100, e.height)
                # Drop shadow offset
                shadow.place(x=6, y=8, width=w, height=h)
                # Main card slightly inset from shadow
        ...
        # Root container
        home_frame = tk.Frame(root, bg=DEEP_SEA_THEME['primary_bg'])
    ...
    def open_stock_layout_with_symbol(symbol):
        # Clear home page and create trading interface
        for widget in root.winfo_children():
            widget.destroy()
    ...
    def create_trading_interface(symbol, ticker, prev_close, latest_close, percent_gain, ta_signal, rsi_val, analysis):
        ...
        top_control_bar = tk.Frame(root, bg=DEEP_SEA_THEME['secondary_bg'], height=40)
        ...
        side_panel = tk.Frame(root, bg=DEEP_SEA_THEME['secondary_bg'], bd=0, highlightthickness=0, width=side_panel_width)
        ...
        add_tab_btn = tk.Button(root, text="+ Add Tab", command=add_tab, font=("Segoe UI", 11, "bold"), 
                               bg="#4CAF50", fg="#E8F5E8", activebackground="#66BB6A", activeforeground="#FFFFFF", 
                               bd=2, relief="raised")
        add_tab_btn.pack(side=tk.TOP, fill=tk.X)
        ...
        close_tab_btn = tk.Button(root, text="X Close Tab", command=close_tab, font=("Segoe UI", 11, "bold"), 
                                 bg="#D32F2F", fg="#FFEBEE", activebackground="#F44336", activeforeground="#FFFFFF", 
                                 bd=2, relief="raised")
        close_tab_btn.pack(side=tk.TOP, fill=tk.X)
